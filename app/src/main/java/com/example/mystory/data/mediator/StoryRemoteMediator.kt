...
class StoryRemoteMediator(
    ...
    override suspend fun load(
        ...
        return try {
            val responseData = apiService.getStoriesWithPage(page, state.config.pageSize)
            val listStoryItem = responseData.listStory
            var listStoryEntity = mutableListOf<StoryEntity>()
            for(item in listStoryItem) {
                listStoryEntity.add(item.toEntity())
            }

            val endOfPaginationReached = listStoryItem.isEmpty()
            ...
            database.withTransaction {
                ...
                val keys = listStoryEntity.map {
                    RemoteKeys(id = it.id, prevKey = prevKey, nextKey = nextKey)
                }
                database.remoteKeysDao().insertAll(keys)
                database.storyDao().insertList(listStoryEntity)
            }
